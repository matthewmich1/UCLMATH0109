...
def lift : (V →ₗ[ℝ] W) →ₗ[ℝ] ((X → V) →ₗ[ℝ] (X → W)) where
  toFun φ := {
    ...
    map_add' := by
      intro _ _
      simp only [map_comp_add]
    map_smul' := by
      intro _ _
      ext
      simp only [Function.comp_apply, Pi.smul_apply, LinearMapClass.map_smul, RingHom.id_apply]
  }
  map_add' _ _ := rfl
  map_smul' _ _ := rfl
...
lemma lift_comp_lift (φ : U →ₗ[ℝ] V) (ψ : V →ₗ[ℝ] W) :
    (lift ψ) ∘ₗ (lift φ) = lift (ψ ∘ₗ φ) := rfl


-- # 4
lemma lift_comp_const (φ : V →ₗ[ℝ] W) : (lift φ) ∘ₗ const = const ∘ₗ φ := rfl
...
lemma d_zero_apply (f : V_[0]) : ∂ f x = f := rfl
...
-- # 5
lemma d_succ_apply (f : V_[n+1]) : ∂ f x = f - ∂ (f x) := rfl



-- # 6
lemma d_one_apply (f : V_[1]) : ∂ f x₀ x₁ = f x₁ - f x₀ := rfl
...
lemma d_two_apply (f : V_[2]) : ∂ f x₀ x₁ x₂ = f x₁ x₂ - f x₀ x₂ + f x₀ x₁ := by
  rw [d_succ_apply, Pi.sub_apply, Pi.sub_apply, d_one_apply, sub_add]
...
-/
lemma d_three_apply (f : V_[3]) : ∂ f x₀ x₁ x₂ x₃ = f x₁ x₂ x₃ - f x₀ x₂ x₃ + f x₀ x₁ x₃ - f x₀ x₁ x₂ := by
  rw [d_succ_apply, Pi.sub_apply, Pi.sub_apply, Pi.sub_apply, d_two_apply]
  abel
...
lemma Lemma5 : (∂[n + 1] ∘ₗ ∂[n] : V_[n] →ₗ[ℝ] V_[n+2]) = 0 := by
  induction n with
  | zero =>
    rw [d_succ, sub_comp, d_zero, lift_comp_const, sub_eq_zero]
  | succ n ih =>
    rw [d_succ, sub_comp]
    nth_rw 3 [d_succ]
    rw [comp_sub, lift_comp_lift, ih, lift_comp_const, map_zero, sub_zero, sub_self]
...
lemma Lemma6 (f : V_[n + 1]) (x : X) : ∂ f = 0 ↔ f = ∂ (f x) := by
  constructor <;> intro h
  · rw [←sub_eq_zero, ←d_succ_apply, h, Pi.zero_apply]
  · rw [h, ←LinearMap.comp_apply, Lemma5, zero_apply]
...
lemma Lemma7 : range ∂[n] = ker ∂[n + 1] :=
  le_antisymm (range_le_ker_iff.mpr Lemma5) (fun f hf ↦ ⟨f 0, ((Lemma6 f 0).mp hf).symm⟩)
