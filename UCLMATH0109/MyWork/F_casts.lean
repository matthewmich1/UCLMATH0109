...
by
  push_cast --could've just used norm_cast to close the goal
  rfl
...
by --because we casted a as int, all other terms become integers, so -b is possible.
...
-- 09
example (a b : ℕ) (h : a ≤ b) : (b - a : ℕ)  = (b : ℤ)  - a:= -- only true in ℕ because of h : a ≤ b
by
  norm_cast
...
by --definition of a | b is ∃ c, a * c = b, so we can use obtain
  obtain ⟨c,rfl⟩ := h
  push_cast
  use (-c)
...
by
  ...
  push_cast
  apply le_min ha hb
-- could do it in one line using apply le_min <;> norm_cast
