...
by
  exfalso
  exact lt_irrefl x (lt_trans h1 h2)
...
example (h1 : x < y) (h2 : y ≤ z) : x ≠ z:=
by
  exact ne_of_lt (gt_of_ge_of_gt h2 h1)
...
by
  exact add_le_add_left (h2.trans h1) x
...
by
  exact add_le_add (h2.trans h1) (h1.trans h3)
...
by
  apply le_trans
  · exact abs_sub_le x y u
  · rw [add_assoc]
    exact add_le_add_left (abs_sub_le y z u) (|x - y|)
