...
by
  ext x
  constructor
  · intro hst --or statement so we use cases with tactic
    cases hst with
    | inl h =>
      right; exact h
    | inr h =>
...
open Set
-- 07
example (A : Type) (x : A) :  x ∈ univ := --universal set on type A is the set containing all terms of type A
by
  trivial
