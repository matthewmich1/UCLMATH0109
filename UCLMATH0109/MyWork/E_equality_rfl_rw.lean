...
by
  rw [h1,h2] -- we get a syntactic equality k+k+k = k+k+k, LEAN does automatic rfl.
...
by
  rw [hcomm] --Lean chooses values to sub in for i and j. Could've also done `exact hcomm 7 n`

-- 13
example (hcomm: ∀ (i j : ℕ), i + j = j + i) : k + m + n = n + (m + k) :=
by
  -- Original goal ⊢ (k + m) + n = n + (m + k); hover mouse over goal to see brackets on LHS
  rw [hcomm] --when applying hcomm, it takes i = (k+m), j = n
