...
by
  intro x --x is elm. of A, goal changes now.
  constructor
  · apply hp
  · specialize hq x; exact hq
...
by --can't use 'use' straight away as a isn't defined.
  obtain ⟨a,ha⟩ := h
  use a --changes goal to P a ∨ Q a
  left; exact ha
