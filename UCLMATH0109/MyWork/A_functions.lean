...
by
  exact f a --This is one way to complete the goal. exact f a also works since a is a term of type A.
...
by
  apply g --We work backwards. Our goal is C and we have g : B → C so if we can prove B we are done.
  apply f --Now our goal is B and we have f : A → B so if we can prove A we are done.
  exact a --Now our goal is A and we have a : A so we are done.
...
example (A: Type) : A → A :=
by
  intro x --Introduces new variable x of type A. Now the goal is to prove A.
  exact x --We have a term x in A now, so we just use exact x to finish the proof.


-- 09
example (A B: Type) (b : B) : A → B :=
by
  intro a --now we have (x : A), so the goal is to prove B.
  exact b
...
by
  intro h  --h is a term of A -> B so the goal is now to prove (C → D).
  exact g --g is a term of C -> D so we are done.
...
-- 12
example (A B C : Type) (f : A → B → C) (x : A) (y : B) : C := --f maps terms of type A to functions from type B to type C
by
  apply f --we have 2 goals now: A and B. So we need to give a term of type A and a term of type B.
  · exact x
  · exact y
