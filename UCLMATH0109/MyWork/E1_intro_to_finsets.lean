...
by
  ...
  apply h.mono -- if we know s is contained in t and s is nonempty then t must be nonempty
...
by
  by_cases h : s.Nonempty -- if a set is nonempty it has a maximum.
  · use s.max' h -- h is there because when we use s.max' we need to give a proof that s is nonempty (since use s.max' becomes a ∀ statement).
    exact fun x a ↦ le_max' s x a
  · -- Since s is empty it is trivially bounded above by 0
    use 0 --can use any positive value.
    intro x hx --we have x in s but x is nonempty (hence contradiction but lean doesn't recognise this.)
    exfalso
    apply h
    use x
...
--prove x is in the finite union of finite sets indexed by I iff x is a member of one of these sets.
by
  exact mem_biUnion
...
--prove that |⋃ᵢⁿ Sᵢ| = |S₁| + ... + |Sₙ|
--disjoint means no intersection, so the sets are different as they don't share any elements.
by
