...
theorem succ_ne_zero (n : N) : n.succ ≠ 0 := --same as (n.succ = 0) → False
by
  intro hf
  contradiction
...
by
  induction m with
  | zero =>
    rw [zero_eq, mul_zero,mul_zero,zero_add]
  | succ m ih =>
    rw [mul_succ,ih,mul_succ,succ_eq_add_one,succ_eq_add_one,add_assoc,add_comm (m),add_assoc, add_comm 1,add_assoc]
...
by
  ...
  | zero =>
    rw [zero_eq,zero_mul]
  | succ n ih =>
    rw [succ_mul,ih,succ_eq_add_one]
...
by
  induction a with
  | zero =>
    rw [zero_eq,zero_mul,zero_mul,zero_mul]
    rfl
  | succ n ih =>
    rw [succ_mul,succ_mul,succ_mul,ih, add_assoc, add_comm (n*c), add_assoc, add_comm c, add_assoc]
...
by
  induction a with
  | zero =>
    rw [zero_eq,mul_zero,mul_zero,mul_zero,zero_add]

  | succ n ih =>
    rw [mul_succ,mul_succ,mul_succ,ih,add_assoc,add_assoc,←add_assoc (c*n),add_comm (c*n),add_assoc]
...
by
  ...
  | zero =>
    rw [zero_eq,zero_mul,zero_mul,zero_mul]
  | succ n =>
    rw [succ_mul,succ_mul,add_mul,mul_assoc] 
...
by
  cases n with
  | zero =>
    rw [zero_eq,one_eq_succ_zero,pow_succ,pow_zero,mul_one]
  | succ n =>
...
by
  cases n with
  | zero =>
    contradiction --from pow definition, any number to the power of 0 is 1.
  | succ n =>
