...
-/
lemma pow_add (x : G) (n m : ℕ): x ^ (n + m) = (x ^ n) * (x ^ m) :=by
-- We use induction on `n` because `x ^ (n + 1)` is defined as `x * x ^ n`
-- and we have the lemma `pow_succ` that allows us to rewrite this easily
-- then `x ^ (n + 1 + m) = x * x ^ (n + m) = x * (x ^ n * x ^ m) = x ^ (n + 1) * x ^ m`
  induction n with
  | zero =>
...
lemma pow_mul_pow_comm (x : G) (n m : ℕ) : x ^ n * x ^ m = x ^ m * x ^ n :=by
  rw [← pow_add, ← pow_add, add_comm]

lemma pow_mul (x : G) (n m : ℕ) : x ^ (n * m) = (x ^ n) ^ m :=by
  induction m with
  | zero => rfl
  | succ m ih =>
    rw [pow_succ, ←ih, add_comm, mul_add, mul_one, pow_add]

lemma inv_pow (x : G) (n : ℕ) : x⁻¹ ^ n = (x ^ n)⁻¹ :=by
  induction n with
  | zero => rw [pow_zero, pow_zero, inv_one]
  | succ n ih =>
    rw [pow_succ, pow_succ,inv_mul, ih, ← inv_mul,  ← inv_mul, ← pow_succ, pow_add, pow_one]


/-
Here is a harder example.
-/
lemma my_lemma (x y : G) (h : x * y = y⁻¹ * x) : x^2 * y = y * x^2 :=by
  have h2 : y ⁻¹ * x⁻¹ = x⁻¹ * y := by
    rw [←inv_mul]
    apply inv_eq
    rw [h, ←ax_assoc, ax_assoc y⁻¹, ax_mul_inv, ax_mul_one, ax_inv_mul]
  have h3 : y⁻¹ = x⁻¹ * y * x := by
    rw [← h2, ax_assoc, ax_inv_mul, ax_mul_one]
  rw [pow_two,ax_assoc,h, h3, ← ax_assoc, ← ax_assoc, ← ax_assoc,ax_mul_inv, ax_one_mul,ax_assoc]
...
example (x y : G) (n : ℕ) (h : x * y = y⁻¹ * x) :
  x^(2*n) * y = y * x^(2*n) :=by
  induction n with
  | zero => rw [pow_zero, ax_mul_one, ax_one_mul]
  | succ n ih =>
    rw [mul_add, pow_add, ←ax_assoc, ← ih, mul_one,ax_assoc, my_lemma _ _ h, ax_assoc]
